...
**AGENT_MEMORY.md 字符数硬性规定**：主文件总字符数不得超过 **5,000**（Unicode 码点口径，UTF-8 读入后 `len()` 判定；`wc -m` 在 C locale 下返回字节数约为其 1.5 倍，不得用作判定依据）。

- 超限动作 = 轮转而非拒写：把最旧的活跃条目整体移入 `docs/memory/2026-MM.md` 归档（该月无归档文件则创建），主文件保持滚动窗口。
- 新条目模板：一行结论 + 关键数字 / SHA / 路径锚点 + 指针；**过程叙事直接写归档，不进主文件**。
- 历史细节一律对归档目录全文检索获取，不在主文件里堆叠。
2. `STARTUP.md`

...
- **理论级深度分析与文献锚定（Literature Grounding）**：在模型选择理由和求解思路中，强制增加理论深度分析：
  - 针对微观填充/空间逾渗问题，强制引入权威物理/数学理论（如 **Balberg 排除体积理论** *Phys. Rev. Lett.* 1984、**Lorenz & Ziff 连续球体逾渗基准** *J. Chem. Phys.* 2001、**Spherocylinder 等效胶囊体几何模型**），并在方案中提前规划文末参考文献与行内引用。
  - 深入讨论算法在处理非线性或高维数据时的收敛速度、步长惩罚、“维度诅咒（Curse of Dimensionality）”与稳健性。
- **统计学硬门禁与序贯公平性（Statistical Rigor & Fair Sampling Scale）**：
  - 所有蒙特卡洛模拟或抽样算法，**必须规划输出 Wilson 95% 置信区间**；
  - 可行性判据必须以 **Wilson 95% 置信下限 $P_{\\text{low}} \\ge P_{\\text{target}}$ 作为可靠性准入硬约束**（禁止仅使用易受抽样噪声干扰的点估计）；
  - **样本量公平性与自适应序贯蒙特卡洛（Sequential MC / SPRT）**：对比基准解与竞争方案时必须保证统计检力的一致性（禁止对最优解用 $M=5000$ 抬高下限，而对挑战方案仅用 $M=200$ 粗筛）。规划粗筛 $M_1=200$，对接近阈值的临界挑战点（$P_{\\text{high}} \\ge P_{\\text{target}}$）自适应加密至 $M_2=1000 \\to M_3=5000$，确保所有最终竞争点达到同等级统计精度。
  - 关键临界阈值必须在邻近前置点提供“置信下限不达标”的负向对照证据（证伪最小性）。
- **全局最优性证书协议（Optimality Certificate Protocol）**：
  - 双变量成本优化问题必须规划**一维上确界前沿排除证明算法**（如 $N_B^{\\max}(N_A) = \\lfloor \\frac{C^* - c_A N_A - \\varepsilon}{c_B} \\rfloor$），并要求代码手生成对应的边界排除证书 CSV（如 `ques4_global_frontier_certificate.csv`）进行全量断言。
  - 规范学术措辞：严谨陈述为“在 Wilson 95% 置信下限准入规则及同等统计精度下完成了一维边界排除证明”，杜绝脱离统计假设的空泛断言。

## 每个问题的建模方案必须包含
1. **问题类型判断**：属于预测/评价/分类/优化中的哪一类
2. **模型选择理由**：为什么选这个模型，相比备选方案的优势。必须深入讨论该算法在处理非线性或高维数据时的收敛性、理论基准与维度惩罚。

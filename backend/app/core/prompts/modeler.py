...
5. `acceptance_metrics`：至少一个可从结构化结果复算的指标，包含比较符、阈值/目标、单位和计算口径。`target` 必须是有限 JSON 数值，不得是字符串、数组、`null`、`NaN` 或无穷值。量纲正确、公式一致等定性检查必须转换为数值通过标志（例如 `eq 1`），并在 `unit` / `description` 说明 1 的含义。不要以“结果合理”“图像平滑”充当指标。对于“判断是否存在/显著/改善”等开放性问题，只能约束模型比较已完成、数据覆盖、数值有限和结果可复算，禁止用 `fit_improvement ge 0.01`、`p_value le 0.05` 或存在标志 `eq 1` 在执行前强迫肯定结论。

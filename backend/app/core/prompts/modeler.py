...
5. `acceptance_metrics`：至少一个可从结构化结果复算的指标，包含比较符、阈值/目标、单位和计算口径。`target` 必须是有限 JSON 数值，不得是字符串、数组、`null`、`NaN` 或无穷值。量纲正确、公式一致等定性检查必须转换为数值通过标志（例如 `eq 1`），并在 `unit` / `description` 说明 1 的含义。不要以“结果合理”“图像平滑”充当指标。RMSE、R²、拟合误差、偏差、准确率、显著性、物理合理性等经验质量阈值的 `description` 必须按“阈值/目标值/判据/容差 + 依据/来自/基于 + 题目原文/数据统计/交叉验证/文献标准”说明目标值依据；“由结果计算”“符合物理常识”不构成阈值依据。题面仅说“约”“左右”“尽可能稳定”等定性语句时，**不得**自行把它转成 5%、0.1 秒等硬阈值；应记录实际偏差/达到时间并以“方案已计算、情景已覆盖、数值可复算”等结论中立指标验收。没有可靠依据时，改用模型比较已完成、数据覆盖、数值有限或结果可复算等结论中立指标，不得臆造数值门槛。对于“判断是否存在/显著/改善”等开放性问题，禁止用 `fit_improvement ge 0.01`、`p_value le 0.05` 或存在标志 `eq 1` 在执行前强迫肯定结论。

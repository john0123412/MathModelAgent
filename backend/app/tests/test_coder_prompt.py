...
class CoderPromptTest(unittest.TestCase):
    ...
    def test_modeler_prompt_requires_metric_target_bases(self):
        ...
        self.assertIn("cv_r2_avg", MODELER_PROMPT)
        self.assertIn("R²/R2", MODELER_PROMPT)
        self.assertIn("RMSE", MODELER_PROMPT)
        self.assertIn("拟合误差/拟合优度", MODELER_PROMPT)
        self.assertIn("显著性（`p值/p_value`）", MODELER_PROMPT)
        self.assertIn("偏差/bias", MODELER_PROMPT)
        self.assertIn("准确率/accuracy", MODELER_PROMPT)
        self.assertIn("文献标准规定回归模型至少应优于常数预测", MODELER_PROMPT)
        self.assertIn("五折交叉验证平均 R² 阈值依据", MODELER_PROMPT)
        self.assertIn("step_grid_convergence", MODELER_PROMPT)
        self.assertIn("iteration_convergence", MODELER_PROMPT)
        self.assertIn("conservation_residual", MODELER_PROMPT)
        self.assertIn("solver_status", MODELER_PROMPT)
        self.assertIn("injector_phase_offset_ms", MODELER_PROMPT)
        self.assertIn("alternate_phase_objective", MODELER_PROMPT)

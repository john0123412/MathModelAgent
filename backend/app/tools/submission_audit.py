...
HIGH_CONF_PHONE_RE = re.compile(
    # 前后视断言同时排除数字与十六进制字符：SHA-256 等哈希串里偶然出现的
    # 11 位数字序列（如 ...c4b18170548590c38a...）不是电话号码。
    r"(?<![0-9a-fA-F])(?:1[3-9]\d{9}|\+?86[- ]?1[3-9]\d{9})(?![0-9a-fA-F])"
    r"|(?:电话|手机|Tel|Mobile|Contact)[:：\s]*\+?\d[\d\s-]{6,}"
)

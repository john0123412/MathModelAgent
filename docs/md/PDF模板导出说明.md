...
导出前处理会对算法复杂度、单目标 Pareto 术语、影子价格外推范围、未实现算法声明和低信息关键词组合进行可解释的条件提示。该检查用于减少学术表述错误和模板化风险，不是 AI 检测、查重或原创性判定。`Creator`/`Producer` 保留真实 Pandoc/XeLaTeX 导出链信息，不通过 PDF 元数据清洗伪装人工排版；正式提交仍以正文、源码、执行证据、AI 工具使用声明和人工复核为准。
...
2026-08-24）：在参考文献前插入"AI 工具使用声明"、按 `[编号] 工具名称，版本/型号，
开发机构/公司，使用日期` 格式顺延追加 AI 工具文献条目、为全部 python 附录围栏前置
声明注释、追加"附录C AI 工具使用详情"表；预检 `ai_disclosure` 项对 huashubei 强制
校验上述四要素。条款引用统一采用不依赖具体条款号的中性表述（官方章程编号未经原文
核实），`_heal_huashubei_disclosure_numbering` 负责幂等修复历史文本中的条款号不一致；
附录C 结果表题统一为"AI 工具使用情况汇总"。若队伍后续核实官方条款号，在该函数恢复
编号即可全链路一致。
...
- 对正文（不含附录和代码块）图片，若其邻近文字没有对应的“图N”引用，后处理会在图片前
  插入中性图号说明；该补齐是幂等的，不重复添加。它只闭合版式交叉引用，人工仍须核对图号、
  图题与正文论证是否语义一致。

...
- 参考文献必须真实存在且可核验：每条引用先经 `5writing/scripts/paper_search.py` 的 search -> verify -> bib 三步从真实 DOI 生成，禁止凭记忆手写题名 / 作者 / 期刊 / 卷期页。查不到就换检索词重查或如实少引，不要为显得学术而编造引用。
...
- 参考文献可回溯性抽查：从参考文献中抽取带 DOI 的条目（不超过 5 条，条目少于等于 5 条时全抽），逐条运行 `5writing/scripts/paper_search.py verify --doi <DOI>`；任一 DOI 无法核验且无人工来源确认记录，判定硬错误 FAIL。无 DOI 条目须有可人工核对的真实出处；全部条目均无 DOI 且无出处记录同样判 FAIL。
